...
static const char *HANDLER_SRC =
    ...
    "        # Return structured error so hosts can display type, location, and\n"
    "        # full traceback separately rather than parsing a bare string.\n"
    "        _tb = _e.__traceback__\n"
    "        # Walk to the innermost frame to find the user-script line number.\n"
    "        while _tb and _tb.tb_next:\n"
    "            _tb = _tb.tb_next\n"
    "        result = {\n"
    "            'error':      str(_e),\n"
    "            'error_type': type(_e).__name__,\n"
    "            'lineno':     _tb.tb_lineno if _tb else None,\n"
